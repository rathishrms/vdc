...
Describe "Template: $template - Tier1 Storage Accounts" -Tags Unit {
	...
	Context "Template and Parameter Compactibility" {
		...
		BeforeEach {
			...
			ForEach ( $File in (Get-ChildItem "$here\..\*vdc*.deploy*.json" | Select-Object  -ExpandProperty Name) ) {
				$Module.Template = $File
			}

			ForEach ( $File in (Get-ChildItem "$here\..\*vdc*.parameters*.json" | Select-Object  -ExpandProperty Name) ) {
